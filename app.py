...
KATEGORI_PENGELUARAN = sorted([
    ...
    "Snacks", "Food/Groceries", "Self Development", "Reimbursement", "Unexpected", "Essentials", "Entertainment",
    KATEGORI_TOP_UP,
...
def halaman_daftar_transaksi():
    ...
    df_display = df_filtered.sort_values(by=['tanggal','id'], ascending=[False, False]).reset_index(drop=True)
    df_display.insert(0, 'No.', range(1, len(df_display) + 1))
    df_display[COL_NOMINAL] = df_display[COL_NOMINAL].apply(lambda x: f"{x:,.0f}".replace(',', '.'))
